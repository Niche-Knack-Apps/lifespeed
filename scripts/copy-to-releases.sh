...
if command -v npx &> /dev/null && [[ -d "android" ]]; then
    ...
    (cd android && ./gradlew assembleRelease bundleRelease) || echo "  Warning: Android build failed (check signing config)"

    # Copy APK/AAB to releases (may be empty if build failed)
